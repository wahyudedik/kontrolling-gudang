            ...
            <div class="bg-white shadow-sm rounded-lg p-4 mb-6">
                <form method="GET" class="flex flex-col sm:flex-row gap-4">
                    <select name="type" class="flex-1 rounded-md border-gray-300">
                        ...
                        <option value="man_power" {{ request('type') == 'man_power' ? 'selected' : '' }}>Man Power
                        </option>
                        <option value="finish_good" {{ request('type') == 'finish_good' ? 'selected' : '' }}>Finish Good
                        </option>
                        <option value="raw_material" {{ request('type') == 'raw_material' ? 'selected' : '' }}>Raw
                            Material</option>
                        <option value="gudang" {{ request('type') == 'gudang' ? 'selected' : '' }}>Gudang</option>
                        <option value="supplier_datang" {{ request('type') == 'supplier_datang' ? 'selected' : '' }}>
                            Supplier Datang</option>
                    </select>
                    <button type="submit"
                ...
                @forelse($todoLists as $todo)
                    <div class="bg-white shadow-sm rounded-lg p-4">
                        <div class="flex justify-between items-start mb-3">
                            ...
                            <span
                                class="px-2 py-1 text-xs rounded {{ $todo->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        ...
                        <div class="space-y-2 text-sm mb-3">
                            <div>
                                ...
                                @endphp
                                <span
                                    class="ml-1 font-medium">{{ $typeLabels[$todo->type] ?? ucfirst($todo->type) }}</span>
                            </div>
                            <div>
                            ...
                            @if ($todo->due_date)
                                <div>
                                    ...
                                    @endphp
                                    <span
                                        class="ml-1 {{ $isOverdue ? 'text-red-600 font-semibold' : ($daysDiff <= 2 ? 'text-yellow-600' : 'text-gray-600') }}">
                        ...
                        <div class="flex gap-2 pt-3 border-t">
                            <a href="{{ route('todo-lists.edit', $todo) }}"
                                class="flex-1 text-center text-blue-600 hover:text-blue-900 text-sm py-1">Edit</a>
                            <form method="POST" action="{{ route('todo-lists.destroy', $todo) }}" class="flex-1">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-full text-red-600 hover:text-red-900 text-sm py-1"
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                    Title</th>
                                <th class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Type
                                </th>
                                <th class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Date
                                </th>
                                <th class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Due
                                    Date</th>
                                <th class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                                    Created By</th>
                                <th class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($todoLists as $todo)
                                <tr>
                                    ...
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap text-sm">
                                        {{ $todo->date ? $todo->date->format('d M Y') : '-' }}</td>
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                        @if ($todo->due_date)
                                            @php
                                                $today = now()->startOfDay();
                                            ...
                                            <div class="flex items-center gap-2">
                                                <span
                                                    class="text-sm {{ $isOverdue ? 'text-red-600 font-semibold' : ($daysDiff <= 2 ? 'text-yellow-600' : 'text-gray-600') }}">
                                                    {{ $todo->due_date->format('d M Y') }}
                                                </span>
                                                @if ($isOverdue)
                                    ...
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex flex-wrap gap-2">
                                            ...
                                            <form method="POST" action="{{ route('todo-lists.destroy', $todo) }}"
                                                class="inline">
                                                @csrf
                                                @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="px-6 py-4 text-center text-gray-500">No To Do Lists found.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
