            ...
            @if ($dailyReport->manPower)
                <div class="bg-white shadow-sm rounded-lg p-4 sm:p-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <p class="text-sm text-gray-600">Karyawan Masuk</p>
                            <p class="text-lg font-medium">{{ $dailyReport->manPower->employees_present }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600">Karyawan Tidak Masuk</p>
                            <p class="text-lg font-medium">{{ $dailyReport->manPower->employees_absent }}</p>
                        </div>
                    </div>
                </div>
            @endif
                ...
                <div class="bg-white shadow-sm rounded-lg p-4 sm:p-6">
                    ...
                    <ul class="list-disc list-inside space-y-1">
                        @foreach ($dailyReport->stockFinishGood as $item)
                            <li class="text-sm sm:text-base">{{ $item->item_name }} - {{ $item->quantity }} karton</li>
                        @endforeach
                    </ul>
                </div>
            @endif
                ...
                <div class="bg-white shadow-sm rounded-lg p-4 sm:p-6">
                    ...
                    <ul class="list-disc list-inside space-y-1">
                        @foreach ($dailyReport->stockRawMaterial as $item)
                            <li class="text-sm sm:text-base">{{ $item->item_name }} - {{ $item->quantity }} kg</li>
                        @endforeach
                    </ul>
                </div>
            @endif
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                        ...
                        @foreach ($dailyReport->warehouseConditions as $condition)
                            <div>
                                ...
                                @if ($condition->check_1)
                                    <p class="text-sm">✓ {{ $checkLabels[1] }}</p>
                                @endif
                                @if ($condition->check_2)
                                    <p class="text-sm">✓ {{ $checkLabels[2] }}</p>
                                @endif
                                @if ($condition->check_3)
                                    <p class="text-sm">✓ {{ $checkLabels[3] }}</p>
                                @endif
                                @if ($condition->check_4)
                                    <p class="text-sm">✓ {{ $checkLabels[4] }}</p>
                                @endif
                                @if ($condition->check_5)
                                    <p class="text-sm">✓ {{ $checkLabels[5] }}</p>
                                @endif
                                @if ($condition->notes)
                    ...
                    <div class="block md:hidden space-y-3">
                        @foreach ($dailyReport->suppliers as $supplier)
                            <div class="border rounded-lg p-3">
                                <p class="font-medium text-gray-900">{{ $supplier->supplier_name }}</p>
                                <p class="text-sm text-gray-600 mt-1">
                                    <span class="font-medium">Jenis Barang:</span> {{ $supplier->jenis_barang ?? '-' }}
                                </p>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="hidden md:block overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($dailyReport->suppliers as $supplier)
                                    <tr>
                                        <td class="px-4 lg:px-6 py-4 whitespace-nowrap">{{ $supplier->supplier_name }}
                                        </td>
                                        <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                            {{ $supplier->jenis_barang ?? '-' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
